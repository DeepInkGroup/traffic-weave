(() => {
  ...
  const state = {
    ...
    cityEvent:null,eventEnd:0,nextEvent:14,eventIndex:0,nextMilestone:25
  };
  ...
  function render(){ctx.setTransform(state.dpr,0,0,state.dpr,0,0);ctx.clearRect(0,0,state.w,state.h);['tunnel','surface','overpass'].forEach(level=>roads.filter(r=>r.level===level).forEach(strokeRoad));drawBuildPreview();drawCars();drawNodes();drawLabels();drawProgress();}

  const capacity=r=>(3+r.lanes*ROAD_TYPES[r.type].perLane+(r.features.transit?7:0))*(r.features.signals?1.25:1)*(r.health<45 ? .78 : 1);
  ...
  function calculateScore(){const designBonus=roads.reduce((n,r)=>n+(r.built?90:0)+(r.features.green?120:0)+(r.features.signals?60:0)+(r.pattern==='s'?50:0)+(r.level==='overpass'?140:r.level==='tunnel'?220:0),0),condition=roads.reduce((sum,r)=>sum+r.health,0)/roads.length;return Math.max(0,Math.round(state.completed*115+state.networkFlow*8+condition*3-Math.max(0,state.averageTime-10)*45+designBonus));}
  ...
  function update(dt) {
    ...
    if(state.spawnClock<=0){spawnCar();state.spawnClock=Math.max(.18,demand*(.72+Math.random()*.55)/(state.cityEvent?.demand||1));}updateCars(dt);roads.forEach(r=>{r.flash=Math.max(0,r.flash-dt*1.7);if(r.traffic)r.health=Math.max(18,r.health-dt*r.traffic*.045*LEVELS[r.level].wear);});
    ...
    if(state.completed>=state.nextMilestone){if(state.mode!=='sandbox')state.budget+=220;toast(`${state.nextMilestone} trips complete · city grant +$220`);tone(760,.15);state.nextMilestone+=25;}
  ...
  function buildCost(){return ROAD_TYPES[state.buildType].cost+PATTERN_COSTS[state.buildPattern]+LEVELS[state.buildLevel].cost+[...state.buildAddons].reduce((sum,key)=>sum+FEATURE_COSTS[key],0);}
  ...
  function maintenanceCost(r){return 120+(r.type==='highway'?100:r.type==='avenue'?50:0)+(r.level==='surface'?0:100);}
