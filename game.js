(() => {
  ...
  const state = {
    ...
    score:0,history:[12.4],nextSample:0,lastFrame:performance.now(),build:false,buildType:'street',buildStart:null,mouse:null
  };

  function makeRoad(a,b,name,curve,type='street',base=1,built=false) {
  ...
  function strokeRoad(r) {
    const g=geometry(r),spec=ROAD_TYPES[r.type],width=spec.width+(r.lanes-spec.lanes)*3;
    ctx.save();ctx.lineCap='round';ctx.lineJoin='round';
    const path=()=>{ctx.beginPath();ctx.moveTo(g.p0.x,g.p0.y);ctx.quadraticCurveTo(g.p1.x,g.p1.y,g.p2.x,g.p2.y);};
    path();ctx.lineWidth=width+7;ctx.strokeStyle=COLOR.ink;ctx.stroke();
    path();ctx.lineWidth=width;ctx.strokeStyle=r.selected?'#f5c65f':spec.surface;ctx.stroke();
    path();ctx.lineWidth=Math.max(4,width-10);ctx.strokeStyle=r.selected?'rgba(255,226,143,.52)':spec.inner;ctx.stroke();
    path();ctx.lineWidth=r.type==='highway'?2.2:1.7;ctx.strokeStyle='rgba(255,255,255,.92)';ctx.setLineDash(r.type==='highway'?[12,7]:[7,9]);ctx.stroke();
    if(r.type==='highway'&&!r.selected){path();ctx.setLineDash([]);ctx.lineWidth=.8;ctx.strokeStyle='rgba(16,45,70,.22)';ctx.stroke();}
    if(r.selected){path();ctx.setLineDash([]);ctx.lineWidth=2;ctx.strokeStyle='rgba(220,143,45,.88)';ctx.stroke();}
    if(r.flash>0){path();ctx.setLineDash([]);ctx.lineWidth=width+14+r.flash*5;ctx.strokeStyle=`rgba(244,186,71,${r.flash*.25})`;ctx.stroke();}
    ctx.restore();
  }
  function drawBuildPreview() {
    if(!state.build||state.buildStart===null||!state.mouse)return;
    const p=nodePoint(state.buildStart),spec=ROAD_TYPES[state.buildType];
    ctx.save();ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(state.mouse.x,state.mouse.y);ctx.lineCap='round';ctx.lineWidth=spec.width+6;ctx.strokeStyle='rgba(16,45,70,.16)';ctx.setLineDash([5,8]);ctx.stroke();ctx.lineWidth=spec.width;ctx.strokeStyle='rgba(39,133,169,.26)';ctx.stroke();ctx.restore();
  }
  function drawNodes() {
  ...
  function drawCars() {
    state.cars.forEach(car=>{const r=roads[car.road];if(!r)return;const g=geometry(r),t=car.direction>0?car.t:1-car.t,p=bezier(g,t),tan=tangent(g,t),angle=Math.atan2(tan.y,tan.x)+(car.direction>0?0:Math.PI),offset=((car.slot%Math.max(1,r.lanes))-(r.lanes-1)/2)*4;
  ...
  const capacity=r=>3+r.lanes*ROAD_TYPES[r.type].perLane;
  const congestion=r=>Math.max(.28,r.traffic/capacity(r));
  const edgeCost=i=>{const r=roads[i],c=congestion(r);return r.base*ROAD_TYPES[r.type].speed*(1+Math.pow(c,2.25)*2.5)/Math.sqrt(r.lanes);};
  ...
  function calculateScore(){return Math.max(0,Math.round(state.completed*115+state.networkFlow*8-Math.max(0,state.averageTime-10)*45+roads.filter(r=>r.built).length*90));}
  function update(dt) {
  ...
  function updateHUD() {
    ...
    $('roadName').textContent=r.name;$('roadType').textContent=ROAD_TYPES[r.type].label.toUpperCase();$('typeSwatch').className=r.type;$('laneCount').textContent=r.lanes;$('trafficCount').textContent=r.traffic;$('roadTravel').textContent=`${(r.base*ROAD_TYPES[r.type].speed*(1+load*load*2.4)*3.2).toFixed(1)}m`;
    ...
    const spec=ROAD_TYPES[r.type],cost=spec.upgrade+(r.lanes-spec.lanes)*120;$('upgradeCost').textContent=sandbox?'FREE':`$${cost}`;$('upgradeButton').disabled=r.lanes>=spec.max||(!sandbox&&state.budget<cost)||state.ended;$('upgradeButton').querySelector('span').textContent=r.lanes>=spec.max?'MAXIMUM WIDTH':'ADD A LANE';drawSpark();
  }

  function roadHit(x,y){let best=-1,dist=Infinity;roads.forEach((r,i)=>{const g=geometry(r);for(let s=0;s<=30;s++){const p=bezier(g,s/30),d=Math.hypot(p.x-x,p.y-y);if(d<dist){dist=d;best=i;}}});return dist<34?best:-1;}
  ...
  function setBuild(active){state.build=active;state.buildStart=null;state.mouse=null;$('buildPanel').hidden=!active;$('buildButton').classList.toggle('active',active);document.body.classList.toggle('building',active);document.querySelector('.play-area').classList.toggle('building',active);$('buildInstruction').textContent='Pick a starting junction';document.querySelectorAll('.step-dot').forEach((el,i)=>el.classList.toggle('active',i===0));}
  function handleBuildNode(i){if(i<0){toast('Tap one of the circular junctions');return;}if(state.buildStart===null){state.buildStart=i;$('buildInstruction').textContent=`From ${nodes[i].label} — choose destination`;document.querySelectorAll('.step-dot').forEach((el,j)=>el.classList.toggle('active',j===1));tone(500,.05);return;}if(i===state.buildStart){toast('Choose a different junction');return;}if(roads.some(r=>(r.a===i&&r.b===state.buildStart)||(r.b===i&&r.a===state.buildStart))){toast('Those junctions are already connected');return;}
    const spec=ROAD_TYPES[state.buildType],sandbox=state.mode==='sandbox';if(!sandbox&&state.budget<spec.cost){toast(`Need $${spec.cost-state.budget} more for that road`);return;}if(!sandbox)state.budget-=spec.cost;const a=state.buildStart,b=i,name=`${nodes[a].label}–${nodes[b].label} ${spec.label}`,p0=nodePoint(a),p2=nodePoint(b),base=Math.max(.55,Math.hypot(p2.x-p0.x,p2.y-p0.y)/360);roads.forEach(r=>r.selected=false);roads.push(makeRoad(a,b,name,0,state.buildType,base,true));state.selected=roads.length-1;roads[state.selected].selected=true;roads[state.selected].flash=1;state.score+=90;toast(`${spec.label} opened: ${nodes[a].label} to ${nodes[b].label}`);tone(680,.14);setBuild(false);updateHUD();}
  ...
  $('buildButton').addEventListener('click',()=>setBuild(!state.build));$('cancelBuild').addEventListener('click',()=>setBuild(false));document.querySelectorAll('.road-type').forEach(btn=>btn.addEventListener('click',()=>{state.buildType=btn.dataset.type;document.querySelectorAll('.road-type').forEach(b=>b.classList.toggle('active',b===btn));state.buildStart=null;$('buildInstruction').textContent='Pick a starting junction';}));
